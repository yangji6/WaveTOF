function [x, con, wtv] = fnlCg_ics_wave_yj(x0,params)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
PCx = params.PSF_shift .* fftc_yj(padarray(Cx, [params.pad_size,0,0,0]), 1);

FTx = params.M3d .* fftc_yj(fftc_yj(PCx,  2),  3) / params.fft_scale;


       
Cx = repmat(dx, [1,1,1,params.num_chan]) .* params.Receive;        

PCx = params.PSF_shift .* fftc_yj(padarray(Cx, [params.pad_size,0,0,0]), 1);

FTdx = params.M3d .* fftc_yj(fftc_yj(PCx, 2),  3) / params.fft_scale;    
...
Fx = params.M3d .* fftc_yj(fftc_yj(PCx, 2), 3) / params.fft_scale;


gradObj = ifftc_yj( params.PSFt_shift .* ifftc_yj(ifftc_yj(params.M3d .* ( Fx  - params.data ), 2), 3), 1 );

gradObj = 2 * sum(params.Ct .* gradObj(1+end/2-params.crop_size/2:end/2+params.crop_size/2,:,:,:), 4) * params.fft_scale;
