function [x, con, wtv] = fnlCg_ics_wave(x0,params)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
PCx = params.PSF_shift .* fft(padarray(Cx, [params.pad_size,0,0,0]), [], 1);

FTx = params.M3d .* fft(fft(PCx, [], 2), [], 3) / params.fft_scale;
...
FTdx = params.M3d .* fft(fft(PCx, [], 2), [], 3) / params.fft_scale;    
...
Fx = params.M3d .* fft(fft(PCx, [], 2), [], 3) / params.fft_scale;


gradObj = ifft( params.PSFt_shift .* ifft(ifft(params.M3d .* ( Fx  - params.data ), [], 2), [], 3), [], 1 );
